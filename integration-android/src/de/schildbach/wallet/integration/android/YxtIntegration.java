 ...
 */
public final class YxtIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/yxt-paymentrequest"; // BIP 71

    /**
     * Request any amount of Yxt (probably a donation) from user, without feedback from the app.
     *
     * @param context
     *            Android context
     * @param address
    ...
    public static void request(final Context context, final String address) {
        final Intent intent = makeYxtUriIntent(address, null);

        start(context, intent);
    }

    /**
     * Request specific amount of Yxt from user, without feedback from the app.
     *
     * @param context
     *            Android context
     * @param address
    ...
    /**
     * Request any amount of Yxt (probably a donation) from user, with feedback from the app. Result
     * intent can be received by overriding {@link Activity#onActivityResult()}. Result indicates
     ...
     *
     * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own Yxt
     * infrastructure and validate the transaction.
     *
     * @param activity
     *            Calling Android activity
     * @param requestCode
     *            Code identifying the call when {@link Activity#onActivityResult()} is called
     *            back
     * @param address
     *            Yxt address
     */
    ...
    /**
     * Request specific amount of Yxt from user, with feedback from the app. Result intent can be
     * received by overriding {@link Activity#onActivityResult()}. Result indicates either
     ...
     * {@link Activity#onActivityResult()}. Result indicates either {@link Activity#RESULT_OK} or
    ...
    /**
     * Put BIP70 payment message into result intent. Meant for usage by Yxt wallet applications.
     *
     * @param result
     *            result intent
     * @param payment
    ...
    /**
     * Get BIP70 payment message from result intent. Meant for usage by applications initiating a Yxt
     * payment.
     *
     * You can use the transactions contained in the payment to validate the payment. For this, you need your
     * own Yxt infrastructure though. There is no guarantee that the payment will ever confirm.
     *
     * @param result
     *            result intent
    ...
    /**
     * Put transaction hash into result intent. Meant for usage by Yxt wallet applications.
     *
     * @param result
     *            result intent
     * @param txHash
    ...
    /**
     * Get transaction hash from result intent. Meant for usage by applications initiating a Yxt payment.
     *
     * You can use this hash to request the transaction from the Yxt network, in order to validate. For
     * this, you need your own Yxt infrastructure though. There is no guarantee that the transaction has
     * ever been broadcasted to the Yxt network.
     *
     * @param result
     *            result intent
    ...
    private static Intent makeYxtUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("yxt:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
    ...
    private static void redirectToDownload(final Context context) {
        Toast.makeText(context, "No Yxt application found.\nPlease install Yxt Wallet.", Toast.LENGTH_LONG)
                .show();
